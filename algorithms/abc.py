...
class BaseESMaster(abc.ABC,protobuf.roll_out_service_pb2_grpc.RollOutServiceServicer):
    ...
    def __init__(self,
        ...
        self._communication_helper = communication.CommunicationHelper(
            workers=workers, logger=logger)
        self._solution = utility.create_solution()
        self._rnd = np.random.RandomState(seed=seed)
        self._algorithm = None

        self._gcs_folder = experiment_name
        try:
            self._bucket = utility.get_gcs_bucket(
                logger=logger,
        ...
        for iter_cnt in range(self._max_iter):
            ...
            if (iter_cnt + 1) % self._eval_every_n_iter == 0:
                ...
                if mean_score > best_eval_score:
                    ...
                    best_so_far = False
                utility.log_scores(logger=self._logger,
                                        iter_cnt=iter_cnt + 1,
                                        scores=eval_scores,
                                        evaluate=True)
                utility.save_scores(log_dir=self._log_dir,
                                         n_iter=iter_cnt + 1,
                                         scores=eval_scores)
                self._save_solution(iter_count=iter_cnt + 1,
                                    best_so_far=best_so_far)
                utility.save_logs_to_gcs(logger=self._logger,
...
class BaseESWorker(abc.ABC,
    ...
    def __init__(self, logger):
        ...
        self._logger = logger
        self._task = utility.create_task(logger=logger)
        self._solution = utility.create_solution()
        self._communication_helper = communication.CommunicationHelper(
            logger=logger)
